...
module.exports = class KickCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (member.user.id === this.client.user.id)
      return msg.reply(":no_entry_sign: I can't kick myself \\:P");
    const botMember = await msg.guild.fetchMember(this.client.user);
    if (!botMember.hasPermission("KICK_MEMBERS"))
      return msg.reply(
        ":no_entry_sign: I don't have **Kick Members** permission!"
      );
    ...
    if (modlogData.modlog === "disabled") {
      ...
      this.client.channels
        ...
        .catch(err => {
          return msg.reply(
            ":no_entry_sign: **Error:** I couldn't send the kick embed in #mod-log. Please make sure I have access to #mod-log!"
          );
        });
      m.edit(
        `**${member.user.username}**#${
          member.user.discriminator
        } has been kicked and logged in the #mod-log.`
      );
    } else {
      m.edit(
        `**${member.user.username}**#${
