...
module.exports = class SetCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      details:
        "Anybody with administrator perms can set certain features: \njoinmessage, joindm, joinrole, and leavemessage.",
      examples: [
        "set joinrole Members",
        "set joinmessage Welcome {user} to the server!",
        "set leavemessage disabled"
      ],
      guildOnly: true,
      args: [
        {
    ...
    function set(feature) {
      if (feature === "JOINMESSAGE") {
        ...
        if (to === "disabled") {
          ...
          fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
          msg.reply(
            `:white_check_mark: The join message is now set to:\n${to}`
          );
        }
      } else if (feature === "LEAVEMESSAGE") {
        if (!data[msg.guild.id])
        ...
        if (to === "disabled") {
          ...
          fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
          msg.reply(
            ":white_check_mark: The leave message is now **disabled**."
          );
        } else {
          data[msg.guild.id].leaveMessage = to;
          fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
          msg.reply(
            `:white_check_mark: The leave message is now set to:\n${to}`
          );
        }
      } else if (feature === "JOINDM") {
        ...
        msg.reply(
          ":no_entry_sign: That's not a valid feature. Avaliable features include:\njoinmessage, joindm, joinrole and leavemessage."
        );
      }
    }
