...
module.exports = class ToggleCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      details:
        "Anybody with administrator perms can toggle the modlog or no invite.",
    ...
    function toggle(feature) {
      if (feature === "NOINVITE") {
        ...
        if (data[msg.guild.id].noinvite === "enabled") {
          ...
          msg.reply(
            ":white_check_mark: The no invite feature is now **disabled**."
          );
        } else if (!data[msg.guild.id].noinvite === "disabled") {
          data[msg.guild.id].noinvite = "enabled";
          fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
          msg.reply(
            ":white_check_mark: The no invite feature is now **enabled**."
          );
        } else {
          data[msg.guild.id].noinvite = "enabled";
          fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
        ...
        if (data[msg.guild.id].modlog === "enabled") {
          ...
          channel.delete().catch(err => {
            console.log(err);
            msg.reply(
              ":no_entry_sign: **Error:** I couldn't delete the #mod-log channel. Make sure I have perms!"
            );
          });
          ...
          msg.guild.createChannel("mod-log", "text").then(modlog => {
            modlog.send(
              `You have enabled the modlog. To disable this and delete this channel, just type \`toggle modlog\``
            );
          });
        } else {
          data[msg.guild.id].modlog = "enabled";
        ...
        msg.reply(
          ":no_entry_sign:" + " Please make sure you entered a valid feature!"
        );
      }
    }
