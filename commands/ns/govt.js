...
module.exports = class GovtCommand extends Command {
  ...
  async run(msg, args) {
    ...
    const result = request.get(
      `https://www.nationstates.net/cgi-bin/api.cgi?nation=${name}&q=govt+name+flag+fullname`
    );
      ...
      .then(res => {
        parseString(res.text, (err, obj) => {
          ...
          embed
            ...
            .addField(
              "Administration",
              obj.NATION.GOVT[0].ADMINISTRATION + "%",
              true
            )
            ...
            .addField(
              "International Aid",
              obj.NATION.GOVT[0].INTERNATIONALAID + "%",
              true
            )
            .addField(
              "Law and Order",
              obj.NATION.GOVT[0].LAWANDORDER + "%",
              true
            )
            .addField(
              "Public Transport",
              obj.NATION.GOVT[0].PUBLICTRANSPORT + "%",
              true
            )
            .addField(
              "Social Equality",
              obj.NATION.GOVT[0].SOCIALEQUALITY + "%",
              true
            )
            .addField(
              "Spirituality",
              obj.NATION.GOVT[0].SPIRITUALITY + "%",
              true
            )
