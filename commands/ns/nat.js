...
module.exports = class TriggeredCommand extends Command {
  ...
  run(msg, args) {
    ...
    const result = request.get(
      `https://www.nationstates.net/cgi-bin/api.cgi?nation=${name}&q=name+gdp+population+currency+animal+region+wa+flag+fullname+motto+influence+census;mode=score;scale=66`
    );
      ...
      .then(res => {
        parseString(res.text, (err, obj) => {
          embed
            ...
            .addField(
              "Endorsement Count",
              Math.round(obj.NATION.CENSUS[0].SCALE[0].SCORE),
              true
            )
            .addField("Link", "http://www.nationstates.net/nation=" + name)
            .setFooter(
              `Generated on ${date} at ${time}. For more extensive information, type //more <nation name>`
            );
          msg.embed(embed);
        });
      })
