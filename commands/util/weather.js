...
const GOOGLE_API = "AIzaSyAYR2othFrLJruy3NyP5EhIanztYuviDWg";
const WEATHER_API = "078a7ce87ba10f4027c487c66e40796a";
...
module.exports = class WeatherCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (!GOOGLE_API)
      return msg.reply(
        "my Commander has not set the Google API Key. Go yell at him."
      );
    if (!WEATHER_API)
      return msg.reply(
        "my Commander has not set the Weather API Key. Go yell at him."
      );
    ...
    const response = await request({
      ...
      headers: { "User-Agent": `LazyBot v3.0.0` },
      json: true
    });

    if (response.status !== "OK")
      return msg.reply(this.handleNotOK(msg, response.status));
    if (response.results.length === 0)
      return msg.reply("your request returned no results.");

    const geocodelocation = response.results[0].formatted_address;
    const params = `${response.results[0].geometry.location.lat},${
      response.results[0].geometry.location.lng
    }`;

    const locality = response.results[0].address_components.find(loc =>
      loc.types.includes("locality")
    );
    const governing = response.results[0].address_components.find(gov =>
      gov.types.includes("administrative_area_level_1")
    ); // eslint-disable-line max-len
    const country = response.results[0].address_components.find(cou =>
      cou.types.includes("country")
    );
    const continent = response.results[0].address_components.find(con =>
    ...
    const chanceofrain =
      Math.round((res.currently.precipProbability * 100) / 5) * 5;
    ...
    cond.src = await fs.readFileAsync(
      path.join(
        __dirname,
        "..",
        "..",
    ...
    humid.src = await fs.readFileAsync(
      path.join(
        __dirname,
        "..",
        "..",
    ...
    precip.src = await fs.readFileAsync(
      path.join(
        __dirname,
        "..",
        "..",
...
//google api key: AIzaSyAYR2othFrLJruy3NyP5EhIanztYuviDWg
//darksky api key: 078a7ce87ba10f4027c487c66e40796a
