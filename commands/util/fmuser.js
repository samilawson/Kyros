...
module.exports = class FMUserCommand extends Command {
    ...
    async run(msg){
        if(!unames[msg.author.id].username){
            msg.channel.send(`@${msg.author.id}, you don't seem to have your username set! Type //register yourlastfmusername to set it!`)
        } else {
            const user = request.get(`http://ws.audioscrobbler.com/2.0/?method=user.getinfo&user=${unames[msg.author.id].username}&api_key=1336029958418997879ebb165f5fbb3f&format=json`)
            user.then((res) => {
