...
module.exports = class ChartCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      memberName: "chart",
      description:
        "Sends your lastfm chart (based on your inputted time period)!",
      ...
      args: [
        {
          key: "period",
          prompt:
            "What time period would you like to display? (Defaults to 7 days)",
          type: "string",
          default: "7day",
          validate: text => {
            if (
  ...
  async run(msg, args) {
    if (!unames[msg.author.id]) {
      ...
      var request = await http.get(
        `http://www.tapmusic.net/collage.php?user=${
          unames[msg.author.id].username
        }&type=${args.period}&size=5x5&caption=true`,
        function(response) {
          response.pipe(file);
          setTimeout(function() {
